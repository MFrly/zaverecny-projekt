...
# ----------------------------
# Pozadí: hvězdy + "nebula" mlha
# ----------------------------
def _make_nebula_surface(w: int, h: int, seed: int = 1234) -> pygame.Surface:
    """
    Vytvoří jednorázově jemnou mlhu jako semi-transparentní vrstvu.
    Je to levné (generuje se jen jednou), pak se jen blitá a lehce posouvá.
    """
    rnd = random.Random(seed)
    surf = pygame.Surface((w, h), pygame.SRCALPHA)

    # pár velkých průhledných kruhů přes sebe = mlha/nebula
    for _ in range(18):
        r = rnd.randint(120, 320)
        x = rnd.randint(-50, w + 50)
        y = rnd.randint(-50, h + 50)
        alpha = rnd.randint(18, 45)
        # barva lehce do modra/fialova
        col = (rnd.randint(30, 70), rnd.randint(40, 90), rnd.randint(70, 140), alpha)
        pygame.draw.circle(surf, col, (x, y), r)

    # jemné rozmazání: “fake blur” přes několik posunů (rychlé a stačí)
...
def _init_stars(count: int, w: int, h: int):
    ...
    for _ in range(count):
        ...
        speed = random.uniform(25, 120)  # px/s
        radius = random.choice([1, 1, 1, 2])  # většina 1px, občas 2px
...
def _draw_background(screen: pygame.Surface, dt: float, stars, nebula: pygame.Surface, t: float):
    ...
    # Základní tmavý gradient (dva obdélníky s alfa)
    ...
    # Hvězdy: padají dolů, lehce se “třpytí”
    for s in stars:
        s[1] += s[2] * dt
        s[4] += dt * 3.2
        if s[1] > h:
...
def run_menu():
    ...
    # --- Titulek přes pygame font (spolehlivé, žádné HTML tagy) ---
    ...
    # Jemný “glow” za titulkem (jednorázově)
    ...
    start_btn = pygame_gui.elements.UIButton(
        pygame.Rect(WINDOW_WIDTH // 2 - btn_w // 2, 220, btn_w, btn_h),
        "Start",
        manager
    )
    coop_btn = pygame_gui.elements.UIButton(
        pygame.Rect(WINDOW_WIDTH // 2 - btn_w // 2, 280, btn_w, btn_h),
        "Co-op",
        manager
    )
    quit_btn = pygame_gui.elements.UIButton(
        pygame.Rect(WINDOW_WIDTH // 2 - btn_w // 2, 340, btn_w, btn_h),
        "Konec",
        manager
    )
                ...
                if event.ui_element == start_btn:
                    ...
                    if coop_win is None:
                        coop_win = pygame_gui.elements.UIWindow(
                            ...
                            manager=manager,
                            window_display_title="Co-op",
                            resizable=False
                        )
                        host_btn = pygame_gui.elements.UIButton(
                            pygame.Rect(20, 40, 180, 40),
                            "Host",
                            manager,
                            container=coop_win
                        )
                        join_btn = pygame_gui.elements.UIButton(
                            pygame.Rect(220, 40, 180, 40),
                            "Join",
                            manager,
                            container=coop_win
                        )
                        pygame_gui.elements.UILabel(
                            pygame.Rect(20, 100, 400, 30),
                            "IP adresa serveru (pro Join):",
                            manager,
                            container=coop_win
                        )
                        ip_entry = pygame_gui.elements.UITextEntryLine(
                            pygame.Rect(20, 130, 380, 36),
                            manager,
                            container=coop_win
                        )
                        ip_entry.set_text("127.0.0.1")
                        connect_btn = pygame_gui.elements.UIButton(
                            pygame.Rect(20, 180, 380, 40),
                            "Připojit (Join)",
                            manager,
                            container=coop_win
                        )
